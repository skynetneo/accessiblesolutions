...
const Explosion = ({ ...props }: React.HTMLProps<HTMLDivElement>) => {
  const getDeterministicValue = (index: number, min: number, max: number) => {
    const seed = Math.abs(Math.sin((index + 1) * 9999));
  ...
  const spans = Array.from({ length: 20 }, (_, index) => ({
    ...
    initialX: 0,
    initialY: 0,
    directionX: getDeterministicValue(index, -40, 39),
    directionY: getDeterministicValue(index + 20, -60, -10),
    duration: getDeterministicValue(index + 40, 50, 200) / 100,
  }));
